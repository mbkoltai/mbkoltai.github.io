...
country_labels_hu <- c(
  ...
  baseline_labels <- c("2004-től*", "2010-től")
  ...
  for (log_flag in c(FALSE, TRUE)) {
    ...
    p <- ggplot(df_plot, aes(x = x, y = y)) +
      ...
      scale_fill_manual(
        name   = legend_ratio,
        values = setNames(c("#E41A1C", "#377EB8"), baseline_labels)
      ) +
      scale_shape_manual(
        name   = legend_ratio,
        values = setNames(c(21, 24), baseline_labels)
      ) +
      # ── log scale if flag set ──────────────────────────────────────────────
      { if (log_flag) scale_x_log10() } +
      { if (log_flag) scale_y_log10() } +
      labs(
        title    = "Növekedési arányok a 2000-es évek közepétől illetve 2010-től 2024-ig",
        subtitle = paste0(
        ...
        caption = paste0(
          ...
          "*Bázisév a medián ekviv. nettó jövedelemnél = 2006, ",
          "medián órabérnél = 2007.\n",
...
country_labels_hu <- c(
  ...
  dataset_labels <- c(
    ...
    gdp_per_hr_worked   = "GDP ledolgozott\nmunkaóránként (OECD)",
  ...
  # old: n_series=3, tile_half_h=0.04 → tile_height=0.08, period_step=0.24
# tiles spanned: -0.08 to +0.08 around y_pos → total height per period = 0.24 ✅

# new with 2 series: we want same total height per period
# so period_step should stay 0.24, but tiles are bigger to fill the space

tile_half_h <- 0.06          # bigger tiles to fill space left by missing 3rd series
n_series    <- 2
tile_height <- tile_half_h * 2   # = 0.12
period_step <- 0.24              # <-- keep same as before so y_limits unchanged
...
series_offsets <- c(
  ...
  KKE9  = recode(setdiff(l_groups$list_cntrs$CEE, 
                         c("Czechia", "Slovenia")),   # <-- English for setdiff
                 !!!country_labels_hu)                # <-- then translate
)
  ...
  # ── also translate cee_filters country names ──────────────────────────────────
    ...
    df_data %>%
      ...
      mutate(
        series_type = case_when(
          ...
          TRUE                             ~ NA_character_   # <-- drop ppp_const
        )
      ) %>%
      ...
      mutate(
        ...
        period_group = ifelse(period_base == "2010", "2010-től", "bázisévtől"),
  ...
  for (cee_choice in c("KKE9", "KKE11")) {
    for (metric_choice in c("foldchange", "absdiff", "both")) {
      ...
      period_levels_plot <- c("2010-től", "bázisévtől")
  ...
  nrow()
      ...
      df_plot <- df_long %>%
        ...
        mutate(
          series_type  = factor(series_type,
                           levels = c("vol_index", "ppp_current")),  # <-- no ppp_const
          metric_type  = factor(metric_type,
                           levels = c("foldchange", "absdiff")),
          period_group = factor(
            ifelse(period_base == "2010", "2010-től", "bázisévtől"),
      ...
      # ── y axis ───────────────────────────────────────────────────────────────
      y_limits <- c(period_step   - tile_height - 0.002,
                    2*period_step + tile_height + 0.002)
