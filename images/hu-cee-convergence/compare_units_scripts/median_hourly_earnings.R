...
output_folder <- "output/compare_units/median_hourly_earnings/"

l_median_hr_earnings <- list()
l_median_hr_earnings$raw_data <- read_csv(
  "data/wages/earn_ses_pub2s$defaultview_linear.csv") %>%
  select(!c(OBS_FLAG,CONF_STATUS,DATAFLOW,`LAST UPDATE`,
            freq,sizeclas)) %>%
  filter(sex %in% "Total" ) %>% # & grepl("National|PPS",unit)
  filter(geo %in% c(l_groups$list_cntrs$CEE,l_groups$comp_groups$EU8 ) ) %>% 
  arrange(geo, TIME_PERIOD) %>%
  rename(country=geo,year=TIME_PERIOD,value=OBS_VALUE) %>%
  select(!sex) %>% relocate(c(country,year),.before = unit)

  
# calculate 2006 eur/LUC exchange rates for those cntrs&yrs not yet in eurozone
l_median_hr_earnings$exch_rates <- tibble(
...
# inflation data
l_median_hr_earnings$HICP <- read_csv(
...
l_median_hr_earnings$data_constant2006 <-       # <-- 2006 to match HICP base
  l_median_hr_earnings$raw_data %>%
  ...
  # -------------------------
  # FLAG: pre-adoption conversion needed?
  # -------------------------
  mutate(needs_conversion = abs(eur_nominal - lcu_nominal) > 0.01) %>%

  # -------------------------
  # JOIN HICP (national, 2006 base)
  # -------------------------
  left_join(
    l_median_hr_earnings$HICP %>%
      select(country, year, hicp_national = value_2006_base),  # <-- 2006
    by = c("country", "year")
  ) %>%

  # -------------------------
  # JOIN EU27 HICP (for PPS deflation, 2006 base)
  # -------------------------
  left_join(
    l_median_hr_earnings$HICP %>%
      filter(country == "European Union - 27 countries (from 2020)") %>%
      select(year, hicp_eu27 = value_2006_base),               # <-- 2006
    by = "year"
  ) %>%
  ...
  # -------------------------
  left_join(
    l_median_hr_earnings$exch_rates %>%                        # <-- updated list
      select(country, eur_LCU_rate),
    by = "country"
  ) %>%

  # -------------------------
  # DEFLATE: constant 2006 prices
  # -------------------------
  mutate(
    lcu_constant2006  = lcu_nominal / hicp_national * 100,     # <-- 2006
    pps_constant2006  = pps_nominal / hicp_eu27     * 100,     # <-- 2006

    effective_rate    = ifelse(needs_conversion, eur_LCU_rate, 1),
    eur_constant2006  = lcu_constant2006 / effective_rate      # <-- 2006
  ) %>%
  ...
  # -------------------------
  select(country, year,
         lcu_nominal, lcu_constant2006,
         eur_constant2006,
         pps_nominal, pps_constant2006) %>%
  pivot_longer(
    cols      = -c(country, year),
    names_to  = "unit",
    values_to = "value"
...
l_median_hr_earnings$eu8_weighted_average <- 
  l_median_hr_earnings$data_constant2006 %>%
...
l_median_hr_earnings$norm_values <- l_median_hr_earnings$data_constant2006 %>%
  # join EU8 averages
  left_join(
    l_median_hr_earnings$eu8_weighted_average,   # your aggregated table
    by = c("year", "unit")  ) %>%
  # compute % of EU8
  mutate(
...
with(list(),{
  ...
  last_year_filt <- l_median_hr_earnings$norm_values %>%
      ...
      pull(last_year)
  min_yr <- max(c(min(l_median_hr_earnings$norm_values$year),2004))
  
  x <- l_median_hr_earnings$norm_values %>%
      select(country, year,unit,value=`% of EU8`) %>%
      filter(year %in% c(min_yr,2010,last_year_filt)) %>%
...
local({
  ...
  for (cee9 in c(T, F)) {
    for (plot_type in c("pct", "abs")) {
      ...
      # DATA PREP
      # -------------------------
      df_data <- l_median_hr_earnings$norm_values %>%          # <-- updated
        filter(!grepl("LCU", currency) &
      ...
      max_year <- max(df_data$year, na.rm = T)               # <-- don't hardcode;
                                                              # data only goes to 2022
      ...
      if (plot_type == "pct") {
        y_col      <- "% of EU8"
        y_lab      <- "% of EU8"
        plot_title <- "% of EU8 (median hourly earnings) – CEE countries"  # <-- updated
      } else {
        y_col      <- "value"
        y_lab      <- "Median hourly earnings (EUR)"            # <-- updated
        plot_title <- paste0("Median hourly earnings – absolute values ",
                             "– CEE countries")                # <-- updated
      }
      ...
      p <- ggplot(
        ...
        aes(
          x     = year,
          y     = !!y_sym, #  / ifelse(grepl("abs", plot_type), 1e3, 1)
        ...
        geom_text(
          data = last_year_df,
          aes(
            x = year + 0.5,   # <-- same nudge, fine for 4yr data
            y = if (plot_type == "pct") {
                  `% of EU8` + ifelse(current_constant == "constant", -6, 5)
                } else {
                  value + ifelse(current_constant == "constant", -1, 1)  #  / 1e3
                  # <-- rescaled: hourly earnings are small numbers
                },
            label = if (plot_type == "pct") {
                      paste0(round(`% of EU8`), "%")
                    } else {
                      paste0(round(value, 1)) #  / 1e3 # <-- 2 decimals: values ~2-10 EUR/hr
                    }
          ),
          vjust       = 0.5,
          size        = 3.5,
        ...
        scale_x_continuous(
          ...
          expand = expansion(c(0.04, 0.05))    ) +
        scale_y_continuous(
          expand = expansion(c(0.04, 0.033)),
      ...
      file_name <- paste0(
        output_folder,
        "median_hr_earnings_dyn",                             # <-- updated prefix
        ifelse(cee9, "_CEE9", ""),
        ifelse(plot_type == "abs", "_abs",  ""), ".png" )
...
l_median_hr_earnings$norm_values %>%
  ...
  summarise(
    ...
    ratio_2022_2010 = last_val / base_2010) %>%
  select(country, unit, ratio_2022_2010) %>%   
  pivot_wider(names_from  = unit,
              values_from = ratio_2022_2010) %>%
...
local({
  ...
  metric_cfg <- list(
    pct = list(
      ...
      title     = "Median hourly earnings as a share of the EU8 average",        # <--
      subtitle  = "Relative change from 2006 or 2010 baseline, in percent",
      x_lab     = "Percent change from baseline",
      file_stub = "median_hr_earnings_pct_change_from_baselines",                # <--
      scales    = "fixed",
    ...
    diff = list(
      ...
      file_stub = "median_hr_earnings_abs_change_from_baselines",                # <--
      scales    = "free_x",
      label     = function(change, base, last) {
        paste0(
          ifelse(change > 0, "+", ""),
  ...
  # last available year for hourly earnings is 2022
  last_year <- max(l_median_hr_earnings$norm_values$year, na.rm = T)   # <-- no 2024 cap needed
  ...
  for (cee9 in c(T, F)) {
    for (metric in names(metric_cfg)) {
      ...
      df_change <- l_median_hr_earnings$norm_values %>%                 # <--
        filter(!grepl("LCU", currency),
               country %in% l_groups$list_cntrs$CEE) %>%
        select(country, year, currency, current_constant,
               value = `% of EU8`) %>%
        group_by(country, currency, current_constant) %>%
...
local({
  ...
  save_plot_flag <- T
  periods <- c("2006-2022", "2010-2022")                              # <-- 2022
  cee_filters <- list(
  ...
  for (filt_name in names(cee_filters)) {
    ...
    df_base <- l_median_hr_earnings$norm_values %>%                   # <--
      filter(!grepl("LCU", unit) &
               country %in% cee_filters[[filt_name]]) %>%
      select(country, year, unit, value = `% of EU8`) %>%
      group_by(country, unit) %>%
      filter(year %in% c(2006, 2010, 2022)) %>%                      # <-- 2022
      summarise(
        val_2006 = if (any(year == 2006)) value[year == 2006] else NA_real_,
        val_2010 = if (any(year == 2010)) value[year == 2010] else NA_real_,
      ...
      mutate(
        ch2006_2022_ratio = ifelse(!is.na(val_2006), val_last / val_2006, NA_real_),  # <-- 2022
        ch2010_2022_ratio = ifelse(!is.na(val_2010), val_last / val_2010, NA_real_),  # <-- 2022
        ch2006_2022_diff  = ifelse(!is.na(val_2006), val_last - val_2006, NA_real_),  # <-- 2022
        ch2010_2022_diff  = ifelse(!is.na(val_2010), val_last - val_2010, NA_real_)   # <-- 2022
      )
    ...
    df_ratio <- df_base %>%
      mutate(
        label_2006 = paste0(
          ifelse(ch2006_2022_ratio > 1, "+", ""),                     # <-- 2022
          round((ch2006_2022_ratio - 1) * 100, 1), "% (",
          round(val_2006, 1), "→", round(val_last, 1), ")"
        ),
        label_2010 = paste0(
          ifelse(ch2010_2022_ratio > 1, "+", ""),                     # <-- 2022
          round((ch2010_2022_ratio - 1) * 100, 1), "% (",
          round(val_2010, 1), "→", round(val_last, 1), ")"
        )
      ) %>%
      select(country, unit, label_2006, label_2010) %>%
      pivot_longer(!c(country, unit), names_to = "period") %>%
      mutate(period = paste0(gsub("label_", "", period), "-2022")) %>% # <-- 2022
    ...
    for (i in seq_along(periods)) {
      df_out <- df_ratio %>%
        ...
        select(-sort_val) %>%
        mutate(across(
          c(constant_PPS, constant_euro_2006, current_PPS),           # <-- euro_2006
      ...
      writeLines(
        html_table,
        paste0(
          output_folder, "html_tables/median_hr_earnings_foldchange_", # <--
    ...
    df_diff <- df_base %>%
      mutate(
        label_2006 = paste0(
          ifelse(ch2006_2022_diff > 0, "+", ""),                      # <-- 2022
          round(ch2006_2022_diff, 1), " (",
          round(val_2006, 1), "→", round(val_last, 1), ")"
        ),
        label_2010 = paste0(
          ifelse(ch2010_2022_diff > 0, "+", ""),                      # <-- 2022
          round(ch2010_2022_diff, 1), " (",
          round(val_2010, 1), "→", round(val_last, 1), ")"
        )
      ) %>%
      select(country, unit, label_2006, label_2010) %>%
    ...
    for (i in seq_along(periods)) {
      df_out <- df_diff %>%
        ...
        select(-sort_val) %>%
        mutate(across(
          c(constant_PPS, current_PPS, constant_euro_2006),           # <-- euro_2006
          ~ paste0("#", rank(-as.numeric(str_extract(., "-?\\d+\\.?\\d*"))), ": ",.)
        ))
      ...
      writeLines(
        html_table,
        paste0(
          output_folder, "html_tables/median_hr_earnings_absdiff_",   # <--
          periods[i], "_", filt_name, ".html"
        )
      )
    }
