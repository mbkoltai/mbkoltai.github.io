...
rm(list = ls())
output_folder <- "output/compare_units/gdp_per_cap/"
# load settings
...
#articles/114943-how-do-you-derive-constant-price-series-in-usd

l_gdp_percap <- list()
# load
l_gdp_percap$abs_values <- bind_rows(
read_world_bank_data(path = paste0("data/gdp-per-capita/",
         "API_NY.GDP.PCAP.PP.KD_DS2_en_csv_v2_4572/",
         "API_NY.GDP.PCAP.PP.KD_DS2_en_csv_v2_4572.csv")) %>%
  mutate(series="usd_ppp_const"),
  # PPP current
read_world_bank_data(path = paste0("data/gdp-per-capita/",
         "API_NY.GDP.PCAP.PP.CD_DS2_en_csv_v2_35/",
         "API_NY.GDP.PCAP.PP.CD_DS2_en_csv_v2_35.csv")) %>%
  mutate(series="usd_ppp_current"),
  # USD constant
read_world_bank_data(path=paste0("data/gdp-per-capita/",
         "API_NY.GDP.PCAP.KD_DS2_en_csv_v2_82/",
         "API_NY.GDP.PCAP.KD_DS2_en_csv_v2_82.csv") ) %>%
  mutate(series="usd_constant"))

### ### ### ### ### ### ### ### ### ### ### ### ### ### 
### ### ### ### ### ### ### ### ### ### ### ### ### ### 
# sense check end-to-start ratios

local({
  cee_cntrs <- l_groups$list_cntrs$CEE
    
df_base <- l_gdp_percap$abs_values %>%
...
l_gdp_percap$norm_values  <- l_gdp_percap$abs_values %>%
  split(.$series) %>%
...
with(list(),{
  ...
  last_year_filt <- l_gdp_percap$norm_values %>%
      ...
      pull(last_year)
  min_yr <- max(c(min(l_gdp_percap$norm_values$year),2004))
  
  x <- l_gdp_percap$norm_values %>%
...
local({
  ...
  for (cee9 in c(T, F)) {
    for (plot_type in c("pct", "abs")) {
      ...
      # DATA PREP
      # -------------------------
      df_data <- l_gdp_percap$norm_values %>%          # <--
      ...
      if (plot_type == "pct") {
        y_col      <- "% of EU8"
        y_lab      <- "% of EU8"
        plot_title <- "% of EU8 (GDP per capita) – CEE countries"          # <--
      } else {
        y_col      <- "value"
        y_lab      <- "GDP per capita (USD, thousands)"                    # <--
        plot_title <- "GDP per capita – absolute values (USD, thousands) – CEE countries" # <--
      }
...
local({
  ...
  metric_cfg <- list(
    pct = list(
      ...
      title     = "GDP per capita as a share of the EU8 average",          # <--
      subtitle  = "Relative change from 2004 or 2010 baseline, in percent",
      x_lab     = "Percent change from baseline",
      file_stub = "gdp_per_cap_pct_change_from_baselines",                 # <--
      scales    = "fixed",
    ...
    diff = list(
      ...
      file_stub = "gdp_per_cap_abs_change_from_baselines",                 # <--
      scales    = "free_x",
      label     = function(change, base, last) {
        paste0(
          ifelse(change > 0, "+", ""),
  ...
  # -------------------------
  last_year_by_series <- l_gdp_percap$norm_values %>%                      # <--
    group_by(series) %>%
  ...
  for (cee9 in c(T, F)) {
    for (metric in names(metric_cfg)) {
      ...
      df_change <- l_gdp_percap$norm_values %>%                            # <--
        filter(region == "CEE") %>%
        select(country, year, series, value = `% of EU8`) %>%
...
local({
  ...
  for (filt_name in names(cee_filters)) {
    ...
    df_base <- l_gdp_percap$norm_values %>%                               # <--
      filter(region == "CEE",
    ...
    for (i in seq_along(periods)) {
      ...
      writeLines(
        html_table,
        paste0(
          output_folder, "html_tables/gdp_per_cap_foldchange_",           # <--
    ...
    for (i in seq_along(periods)) {
      ...
      writeLines(
        html_table,
        paste0(
          output_folder, "html_tables/gdp_per_cap_absdiff_",              # <--
          periods[i], "_", filt_name, ".html"
        )
      )
    }
