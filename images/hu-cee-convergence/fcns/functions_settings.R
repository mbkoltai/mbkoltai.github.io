...
read_world_bank_data <- function(path) {
  read_csv(path, skip = 4) %>% 
    ...
    filter(!grepl("Macao|Hong", country) & !is.na(value))
}
...
normalise_and_filter <- function(df,
                            list_groups,
                            filt_region=NA,
                            fltr_year=2e3) {
  
  if (is.na(filt_region)) {
    filt_region <- names(list_groups$list_cntrs)
  }
  

      df_region <- lapply(names(list_groups$list_cntrs), \(x) 
        data.frame(region = x, country = list_groups$list_cntrs[[x]])
      ) %>% bind_rows()
      
      sel_comps = c("Germany", names(list_groups$comp_groups), "World")
      ...
      # popul weighted average
      df_group_averages <- lapply(names(list_groups$comp_groups), \(x)
          df_join %>%
            filter(country %in% list_groups$comp_groups[[x]]) %>%
      ...
      # View(df_group_averages)
      
      df_out <- bind_rows(
        # countries
        df_join,
        # group averages (population-weighted)
        df_group_averages) %>%
        filter(country %in% c(sel_comps,
                      as.character(unlist(list_groups$list_cntrs))) & 
            year>=fltr_year) %>%
        group_by(year) %>% # ,series
        mutate(
          `% of Germany`    = 100 * value / value[country == "Germany"],
          `% of G7`    = 100 * value / value[country == "G7"],
          `% of EU8`   = 100 * value / value[country == "EU8"],
          # `% of S_EUR4`= 100 * value / value[country == "S_EUR4"],
          # `% of LAT_AM`= 100 * value / value[country == "LAT_AM"],
          # `% of World` = 100 * value / value[country == "World"]
        ) %>% 
        ungroup() %>%
        left_join(df_region) %>%
        relocate(region, .before = country) %>%
        filter(region %in% filt_region)
...
format_html_table <- function(
  ...
  hun_row <- which(df_res == "Hungary")
  n_cols <- ncol(df_res)
  
  
  df_out <- df_res %>% kable(format = "html",
    escape = F,
    align = "c") %>%
  kable_styling(
    bootstrap_options = "condensed",
