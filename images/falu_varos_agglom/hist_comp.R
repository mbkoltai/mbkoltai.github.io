...
local({
  raw <- read_csv("adatok/mek_52_53_nepesseg_meretetkategoriak.csv")
  fmt <- scales::label_number(big.mark = " ")
    ...
    meret == "20000 felett"    ~ "25–50k",    # 3 községek, lump into nearest város band
    meret == "25000-50000"     ~ "25–50k",
    meret == "50000-100000"    ~ "50–100k",
    meret == "100000 felett"   ~ "100k+",
    TRUE ~ NA_character_)

  band_lvls <- c("<1k","1–2k","2–5k","5–10k","10–25k","25–50k","50–100k","100k+","Budapest")

  # band_lvls <- c("<1k","1–2k","2–5k","5–10k","10–25k","25k+","Budapest")
  ...
  d <- raw |>
    ...
    mutate(
      ...
      yr = if_else(year %in% c(1990, 1994), "1990/94", as.character(year)) |>
           factor(levels = c("1949","1990/94"))) |>
    filter(!is.na(band), !is.na(yr)) |>
    summarise(n = sum(n_telepules, na.rm = TRUE), tot = sum(nepesseg),
              .by = c(yr, band, tipus))

  # per-facet national total for shares
  ...
  # y-axis labels: per-facet row totals
  row_tot <- d |>
    summarise(rtot = sum(tot), .by = c(yr, tipus)) |>
    left_join(natl, by = "yr") |>
    mutate(lab = sprintf("%s\n%.2fm (%.1f%%)", tipus, rtot / 1e6, 100 * rtot / natl))

  # use 1949 labels on the shared y-axis (facets share it)
  ylabs <- row_tot |> filter(yr == "1949") |> arrange(tipus) |> pull(lab)

  # annotate per-facet row totals inside panels instead (so both years show their own)
  row_ann <- row_tot |>
    mutate(x = length(band_lvls) + 0.3,
           y = if_else(tipus == "város", 2, 1),
           lab2 = sprintf("%.2fm\n(%.1f%%)", rtot / 1e6, 100 * rtot / natl))
  ...
  p <- ggplot(d, aes(x, y)) +
    ...
    geom_text(aes(label = inside), fontface = "bold", size = 3.5, colour = "grey15") +
    geom_text(aes(label = below), vjust = 1, nudge_y = -0.38,
              size = 3.2, lineheight = 0.9) +
    geom_text(data = row_ann, aes(label = lab2), inherit.aes = FALSE,
              x = row_ann$x, y = row_ann$y,
              size = 3.5, fontface = "italic", hjust = 0, colour = "grey30") +
    scale_x_continuous(breaks = 1:length(band_lvls), labels = band_lvls,
                       position = "top",
                       limits = c(0.5, length(band_lvls) + 1),
                       expand = expansion(0)) +
    scale_y_continuous(breaks = c(1, 2),
                       labels = c("község", "város"),
                       limits = c(0.35, 2.55), expand = expansion(0)) +
    scale_fill_manual(values = c("város" = "#00BFC4", "község" = "#F8766D"),
                      guide = "none") +
    geom_hline(yintercept = 1.35) +
    coord_cartesian(clip = "off") +
    labs(x = NULL, y = NULL,
         title = "Település-kategóriák népessége, 1949 vs 1990/94",
         subtitle = "A körök területe a kategória összlakosságával arányos",
         caption = "1990/94: községek 1990, városok 1994 · Forrás: MEK") +
    theme_bw() +
  ...
  ggsave("plots/hist/mek_bubble_1949_vs_1990.png", p,
         width = 18, height = 7.5, units = "in", dpi = 300, bg = "white")
  print(p)
})
...
local({
  ...
  blabs <- c("<1k","1–2k","2–5k","5–10k","10–25k","25–50k","50–100k","100k+")

  # --- historical (MEK) ---
  d_hist <- raw_mek |>
    filter(meret_kategoria != "együtt", !is.na(nepesseg)) |>
    mutate(
      band = recode_band(meret_kategoria) |> factor(levels = band_lvls),
      tipus = case_when(
        tipus == "község" ~ "község",
        TRUE              ~ "város"),
      yr = if_else(year %in% c(1990, 1994), "1990/94", as.character(year))) |>
  ...
  # --- modern (HNT) ---
  d_hnt <- hu_hnt_lakonepesseg_2012_2025 |>
    filter(year %in% c(2012, 2025)) |>
    mutate(telepules = if_else(grepl("Budapest", telepules), "Budapest", telepules)) |>
    summarise(pop = sum(lakonepesseg), jogallas = first(jogallas), .by = c(year, telepules)) |>
    mutate(
      tipus = case_when(
        jogallas %in% c("község","nagyközség") ~ "község",
        TRUE                                   ~ "város"),
      band = if_else(telepules == "Budapest", "Budapest",
                     as.character(cut(pop, brks, blabs, right = FALSE))) |>
             factor(levels = band_lvls),
      yr = as.character(year)) |>
    filter(!is.na(band)) |>
    summarise(n = n(), tot = sum(pop), .by = c(yr, band, tipus))

  yr_lvls <- c("1949","1990/94","2012","2025")
  d_all <- bind_rows(d_hist, d_hnt) |> mutate(yr = factor(yr, levels = yr_lvls))

  # === plot function ===
  make_bubble <- function(by_type = TRUE, size_by_pct = FALSE) {

    if (by_type) {
      d <- d_all |> filter(tipus != "Budapest")
    } else {
      d <- d_all |>
    ...
    # broad size category for colouring the size-only variant
    d <- d |> mutate(
      szint = case_when(
        ...
        TRUE                                    ~ "falu") |>
        factor(levels = c("falu","kis/középváros","nagyváros","Budapest")))
    ...
    p <- ggplot(d, aes(x, y)) +
      ...
      geom_text(aes(label = below), vjust = 1, nudge_y = ny,
                size = 3.5, lineheight = 0.88) +
      scale_x_continuous(breaks = 1:length(band_lvls), labels = band_lvls,
                         position = "top",
                         limits = c(0.5, length(band_lvls) + if (by_type) 1.5 else 0.5),
                         expand = expansion(0))

    if (by_type) {
      p <- p +
        scale_y_continuous(breaks = c(1, 2), labels = c("község","város"),
                           limits = c(0.3, 2.6), expand = expansion(0)) +
        scale_fill_manual(values = c("város" = "#00BFC4", "község" = "#F8766D"),
                          guide = "none") +
        geom_hline(yintercept = 1.35) +
        geom_text(data = row_tot, aes(label = lab2), inherit.aes = FALSE,
                  x = row_tot$x, y = row_tot$y,
                  size = 5, fontface = "italic", hjust = 0, vjust = 1,
                  lineheight = 0.9) +
      ...
      p <- p +
        ...
        scale_fill_manual(values = c("falu"            = "#F8766D",
    ...
    p <- p +
      ...
      labs(x = NULL, y = NULL,
           ...
           caption = "1990/94: községek 1990, városok 1994 · Korábbi évek: MEK · 2012, 2025: KSH HNT") +
      theme_bw() +
      theme(panel.grid = element_blank(),
    ...
    fname <- sprintf("plots/hist/mek_bubble_%s_%s.png", type_str, size_str)
    ggsave(fname, p, width = 18, height = h, units = "in", dpi = 300, bg = "white")
    print(p)
  }
...
local({
  ...
  d_mek <- raw_mek |>
    ...
    mutate(
      band=recode_band(meret_kategoria) |> factor(levels=band_lvls),
      tipus=case_when(tipus == "község" ~ "község", TRUE ~ "város"),
      yr=if_else(year %in% c(1990, 1994), "1990/94", as.character(year))) |>
    filter(!is.na(band)) |>
    summarise(n=sum(n_telepules, na.rm=TRUE), tot=sum(nepesseg),
              .by=c(yr, band, tipus))
  ...
  yr_lvls <- c("1949","1990/94","2012","2025")
  d_all <- bind_rows(d_mek, d_hnt) |> mutate(yr=factor(yr, levels=yr_lvls))
  ...
  make_trajectory <- function(by_type=FALSE, size_by_pct=FALSE) {
    ...
    # arrows between consecutive time points
    arrows <- d |>
    ...
    max_sz <- if (by_type) 40 else 58
    w <- if (by_type) 18 else 14
    h <- 18
    ...
    p <- p +
      geom_segment(data=arrows,
                   aes(x=yr_pos, y=y, xend=x_end, yend=y_end),
                   colour="grey70", linewidth=0.5,
                   arrow=arrow(length=unit(0.15, "inches"), type="closed"),
                   show.legend=FALSE) +
      geom_point(aes(size=bubble_val, fill=if (by_type) tipus else szint),
                 shape=21, colour="white", stroke=1, alpha=0.9) +
      scale_size_area(max_size=max_sz, guide="none") +
      geom_text(aes(label=inside), size=7, fontface="bold") + # , colour="grey15"
      geom_text(aes(label=sprintf("n=%d", n)), vjust=1, nudge_y=-0.35,
                size=5.5) + # , colour="grey40"
      scale_x_continuous(breaks=1:length(yr_lvls),
                         labels=yr_lvls,
                         position="top",
                         expand=expansion(mult=0.12)) +
      scale_y_continuous(breaks=1:length(band_lvls),
    ...
    type_str <- if_else(by_type, "tipus", "osszes")
    size_str <- if_else(size_by_pct, "pct", "abs")
    sub_str  <- if (size_by_pct) "Buborékméret ∝ országos népesség-részarány" else
                                 "Buborékméret ∝ összlakosság"
    ...
    p <- p +
      ...
      labs(x=NULL, y=NULL,
           ...
           subtitle=paste0(sub_str,
               if (by_type) " · község=község + nagyközség, város=minden városi jogállás" else ""),
           caption="1990/94: községek 1990, városok 1994 · Korábbi évek: MEK · 2012, 2025: KSH HNT") +
    ...
    ggsave(sprintf("plots/hist/mek_trajectory_%s_%s.png", type_str, size_str), p,
...
local({
  ...
  bp_1920 <- 1232026   # from KSH long time series
  ...
  yr_lvls <- c("1920","1949","1990/94","2012","2025")
  d_all <- bind_rows(d_1920, d_mek, d_hnt) |>
  ...
  make_bubble <- function(size_by_pct=FALSE) {
    ...
    p <- ggplot(d, aes(x, y)) +
      ...
      geom_text(aes(label=inside), fontface="bold", size=3.8, colour="grey15") +
      geom_text(aes(label=below), vjust=1, nudge_y=-0.45,
                size=3.2, lineheight=0.88) +
      scale_x_continuous(breaks=1:length(band_lvls), labels=band_lvls,
                         position="top",
  ...
  make_trend <- function(show_pct=TRUE) {
    ...
    p <- ggplot(d, aes(yr_num, value, colour=szint)) +
      ...
      scale_x_continuous(breaks=c(1920, 1949, 1992, 2012, 2025),
                         labels=c("1920","1949","1990/94","2012","2025"),
...
local({
  ...
  make_trajectory <- function(size_by_pct = FALSE) {
    ...
    p <- ggplot(d, aes(yr_pos, y)) +
      ...
      title = "Település-kategóriák népessége, 1920–2025",
      subtitle = sub_str,
      caption = paste0("1920: MEK 51. táblázat · 1949, 1990/94: MEK 52–53. ",
        "· 2012, 2025: KSH HNT\n1990/94: községek 1990, városok 1994")) +
      theme_bw(base_size = 20) +
      theme(panel.grid.major.x = element_blank(),
            panel.grid.minor = element_blank(),
    ...
    ggsave(sprintf("plots/hist/mek_trajectory_1920_%s.png", size_str), p,
