...
If we want to add additional detail to the model, the first thing to add should probably be growing labour productivity that would also lead to expanded reproduction (which means that some of the surplus is recycled to the system, not consumed in the current periods). The other component to add for a bit more realistic model would be fixed capital. If we add these, then it can start to make sense to ask questions on how productivity growth will affect profitability. It is in this direction I will extend the simple model summarized by Equation \eqref{unit_price_dynam_explicit} in the coming posts.
